 ...
 */
func progressUpdate(_ stepsTaken:Int, _ goal:Int) {
    if stepsTaken < goal/10 {
        print("You're off to a good start.");
    }
    else if stepsTaken < goal/2 {
        print("You're almost halfway there!");
    }
    else if stepsTaken >= goal/2 {
        print("You're over halfway there!");
    }
    else if stepsTaken < Int(Double(goal)*0.9) {
...
progressUpdate(3000, 6000);
progressUpdate(2000, 7000);
progressUpdate(7000, 10000);
...
func pacing(_ currentDistance:Double, _ totalDistance:Double, _ currentTime:Double, _ goalTime:Double) -> String {
    let runRate = (currentDistance<totalDistance*(currentTime/goalTime)) ? "You've got to push it just a bit harder!" : "Keep it up!";
    return runRate;
}
print(pacing(currentDistance, totalDistance, currentTime, goalTime));
